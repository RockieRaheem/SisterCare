...
export default function DashboardPage() {
  ...
  const getCountdown = () => {
    if (!cycleInfo) return { days: 0, hours: 0, minutes: 0 };

    const now = new Date();
    const totalMinutesLeft = cycleInfo.daysUntilNextPeriod * 24 * 60;
    const hoursLeft = Math.floor((now.getHours() * 60 + now.getMinutes()) / 60);
    const minutesLeft = 60 - (now.getMinutes() % 60);

    return {
      days: cycleInfo.daysUntilNextPeriod,
  ...
  const currentPhaseInfo = cycleInfo
    ? phaseInfo[cycleInfo.phase]
